...
- **Reuse existing components/patterns — do NOT build a new version from scratch when an equivalent already exists.** This is a hard rule the user has repeated. Before creating a new picker / modal / list / card / chat surface, find the existing one and parameterise/lift it (add a `mode`/`selectionMode` prop, factor out a shared component) rather than re-implementing. Canonical reuses: alter selection → `AlterSearchSelect` / `SetFrontModal` / `GroupMembersModal` (selectionMode); per-alter feelings/symptoms/notes → the `CurrentFronters` `AlterPanel` (EmotionWheelPicker + the 1–5/boolean symptom rows); the alters-page list with subsystem navigation + grid/list view → `AlterGrid`/`AlterGridView`/`GroupFolderView`/`SubsystemAlterList`; the press-and-hold action menu → `AlterActionMenu`; chat composing/rendering + signposts/whispers → the system-chat components (`renderRichContent`, the chat composer) — scope them, don't fork them. When the user says "like the X" or "same as Y", they mean *literally reuse Y's component/logic*, not approximate it. Forking a pattern makes it drift and need re-fixing in N places.
