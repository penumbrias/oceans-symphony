...
export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      { type: "improve", text: "In the planner, holding and dragging out a new plan can now run past midnight — keep dragging sideways into the next day (or the one after) and the plan stretches to match." },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "improve", text: "Sharing a pack now copies a short message explaining what it is and how to use it, with the code at the end — pasting the whole thing still imports fine." },
      ...
      { type: "fix", text: "Bottom bar taps could get eaten — opening the quick-actions row instead of navigating, or needing several tries. Two invisible tap-blockers fixed; the app also now self-heals if a closed popup leaves the screen unresponsive." },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "fix", text: "Removed a leftover Top-bar on/off switch in Appearance that contradicted the edit menu (the top bar always stays — arrange it instead)." },
      { type: "improve", text: "The pinned {{alter}} bar now slides up and down and answers swipe gestures exactly like the quick actions row." },
      { type: "hotfix", text: "Hotfix: retired the old standalone Diary Cards page (diary fields live inside each check-in) and fixed hardcoded {{alter}}/{{system}} words in a few labels." },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "improve", text: "The Background section owns the wallpaper now (including folder rotation), so the Wallpaper pill is gone — and the section's default view is the wallpaper, not the page song. “UI size” is now “UI & text”." },
      ...
      { type: "improve", text: "Profile style editors ({{alter}}, group, location pages) now use the same sections as every other edit menu — UI & text, Colors & background — with sliders behind set buttons. Header/body colors and sync work as before." },
      ...
      { type: "improve", text: "Widget options: appearance controls use the same sections as Display options (UI & text, Colors & background, Presets), with sliders behind set buttons." },
    ],
  },
  {
    date: "August 13, 2026",
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "improve", text: "\ud83e\uddea New UI: widgets center their contents by default, and every widget's options now have an Alignment choice (top / center / bottom) plus a Content size slider that scales everything inside it." },
      ...
      { type: "improve", text: "\ud83e\uddea New UI: the Log-symptoms widget is the check-in's real symptoms section now \u2014 tabs, search, severity anchors, active episodes, per-alter assignment \u2014 with a Save that counts your selections and an \"Undo last save\". Minimal stays one-tap rows, where the tapped value lights up and tapping it again (or Undo) removes the entry." },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "fix", text: "🧪 New UI: the search button in the top bar opens search again." },
      { type: "fix", text: "Widget options: text you type into a widget's options is saved even if you close the panel with Escape." },
  ...
  },
  {
    ...
    changes: [
      ...
      { type: "feature", text: "Folders in the Asset Library can now be assigned to specific alters as owners. Each user-created folder gets a small people-icon button in its header — tap it to pick one or more alters. Once any folder has owners, an \"All owners / <alter>\" filter row appears at the top of the library so you can narrow to just one alter's folders (their mini asset library). Ownership is optional and never auto-assigned. Renaming a folder carries the ownership with it." },
      ...
      { type: "improve", text: "Every word in the terminology preview is now individually tap-editable — including plurals. Support for irregular plurals like child → children, person → people (previously the app just stuck an \"s\" on the end). Base terms auto-switch you into Custom terms when you tap them, so you can go from a preset to a one-word customisation without touching the Custom button first." },
      { type: "improve", text: "Word-form overrides moved into the terminology preview itself — tap the pencil icon on the preview panel, then tap any highlighted word (fronting, fronter, switching) to edit it inline. Replaces the old \"Advanced word forms\" collapsible drawer." },
