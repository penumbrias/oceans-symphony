 ...
 */
export default function BulletinActionMenu({ bulletin, alters = [], open, onClose, onOpen, taskId = null, isUrgent = false }) {
  const qc = useQueryClient();
  const [busy, setBusy] = useState(null); // "board" | "dash" | "urgent" | "delete" | null
  ...
  // For task bulletins: flip the linked Task's is_urgent so it joins (or
  // leaves) the urgent strip at the top of the dashboard.
  async function toggleUrgent() {
    if (!taskId) return;
    setBusy("urgent");
    try {
      await base44.entities.Task.update(taskId, { is_urgent: !isUrgent });
      qc.invalidateQueries({ queryKey: ["tasks"] });
      qc.invalidateQueries({ queryKey: ["bulletins"] });
      toast.success(isUrgent ? "Urgent flag removed" : "Marked urgent");
      onClose();
    } catch (e) {
      ...
      <DialogContent className="max-w-xs">
        <div className="space-y-2">
          {taskId && (
            ...
            >
              <Zap className={`w-4 h-4 ${isUrgent ? "text-amber-500 fill-amber-500/30" : "text-amber-500"}`} />
              <span className="text-sm">{isUrgent ? "Remove urgent flag" : "Mark as urgent"}</span>
              {busy === "urgent" && <Loader2 className="ml-auto w-3.5 h-3.5 animate-spin" />}
            </button>
          )}
          <button
            type="button"
