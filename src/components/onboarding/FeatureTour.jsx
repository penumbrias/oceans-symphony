...
function buildSteps(t, alterId = null, tourAlterWasCreated = false) {
  ...
  return [
    ...
    },
    {
      ...
      emoji: "➕",
      title: "Create a Poll",
      body: `Tap the highlighted Create button to make a new poll. Enter a question and add 2–4 options. Polls have two voting modes: per-${t.alter} (each tile in the avatar grid votes once, tap again to remove) or anonymous tally count (each tap on an option just adds 1, with a − button to subtract). Toggle "Anonymous tally count" at the top of the create modal or on a poll's detail view; whichever mode you last picked becomes the default for the next poll. Polls posted to the Bulletin Board automatically pin themselves to the board's Pinned section so the question is hard to miss; voting is unified between the two surfaces. Tap the Pin icon on a poll's detail view to pin/unpin from the Bulletin Board. Double-tap a poll inside a bulletin to open it on the Polls page.`,
