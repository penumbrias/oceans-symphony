...
export function buildSteps(t, alterId = null, tourAlterWasCreated = false) {
  ...
  return [
    ...
    },
    {
      ...
      body: `The Quick Check-In button opens a multi-section flow for logging emotions, symptoms, activities, and notes in one go. Tap the section pills, or use the ‹ Prev / Next › arrows at the bottom to step through them one at a time. The Feeling section has a side slider that quick-logs a 0–5 rating — Energy level by default; tap its label to track any other rating symptom/habit. A "Who are you with?" pill lets you mark contacts as currently with you, the same way the Fronting pill tracks ${t.alters}. Any ${t.alter} can use it at any time.`,
    ...
    },
    {
      ...
      body: `Press and hold the Quick Check-In button for 3 seconds to pop up your Quick Actions menu — a pill list of shortcuts you've configured, with a wide range of types to choose from (jump to a page, set a specific ${t.alter} to ${t.front}, start or log an activity or symptom, mark a contact as with you, start/end sleep, post a status, add a to-do, and more). Configure them in Settings → Tracking setup → Quick actions. Four more optional buttons can sit in this same row — "Start Activity", "Start Symptom", "Quick Task", and "Quick Plan" — each its own colour, each a fast way to start something or add to your to-do list / schedule without leaving the Dashboard; turn them on in Settings → Appearance → Layout → Dashboard, nested under the Quick Check-In toggle (off by default).`,
