      ...
      <DialogContent className="max-w-md max-h-[90vh] flex flex-col overflow-hidden p-0">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-6 py-4 space-y-4">
          ...
          <div className="space-y-2">
            <button type="button" onClick={() => setShowMembers((s) => !s)}
            ...
            {showMembers && (
              <div className="rounded-xl border border-border/40 overflow-hidden">
                <div className="flex items-center gap-2 px-2.5 py-2 border-b border-border/40 bg-muted/15">
                ...
                <div className="max-h-48 overflow-y-auto overscroll-contain divide-y divide-border/30">
                  {filteredMembers.length === 0 && (
                    <p className="px-3 py-3 text-xs text-muted-foreground">No {t.alters} found.</p>
                  )}
                  {filteredMembers.map((a) => {
                    const checked = selectedMembers.has(a.id);
                    return (
                      <button key={a.id} type="button" onClick={() => toggleMember(a.id)}
                        ...
                        <span className="w-3.5 h-3.5 rounded-full border flex-shrink-0" style={{ backgroundColor: a.color || "transparent", borderColor: a.color || "hsl(var(--border))" }} />
                        <span className="flex-1 min-w-0 truncate text-sm">{formatAlter(a)}</span>
                        <span className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${checked ? "bg-primary border-primary" : "border-border"}`}>
