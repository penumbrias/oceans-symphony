            ...
            <div key={s.id} className="rounded-xl border border-border/60 bg-muted/10 px-3 py-2.5">
              {editing ? (
                ...
                <div className="flex items-center gap-2 min-h-[2.5rem]">
                  ...
                  <p className="flex-1 min-w-0 text-sm font-medium truncate">{displayName(s)}</p>
                  {systems.length > 2 && (
                  ...
                  <button type="button" onClick={() => startRename(s)} disabled={busy} title={`Rename ${terms.system}`}
                    className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors flex-shrink-0 disabled:opacity-50">
                  ...
                  {isActive ? (
                    <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 h-8 rounded-md bg-emerald-500/15 text-emerald-500 text-xs font-semibold">
                      <Check className="w-3.5 h-3.5" /> Active
                    </span>
                  ) : (
                    <Button size="sm" variant="outline" onClick={() => switchTo(s.id)} disabled={busy} className="gap-1.5 flex-shrink-0">
