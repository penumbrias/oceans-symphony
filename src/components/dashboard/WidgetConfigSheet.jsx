...
import { Image as ImageIcon, X, Trash2, ChevronDown, ChevronUp, Check, Eye, EyeOff, GripVertical } from "lucide-react";
...
function ArrangementField({ value = [], onChange, terms }) {
  const formatAlter = useAlterLabel();
  const { data: alters = [] } = useQuery({ queryKey: ["alters"], queryFn: () => base44.entities.Alter.list() });
  const { data: groups = [] } = useQuery({ queryKey: ["groups"], queryFn: () => base44.entities.Group.list() });
          ...
          {(def.configFields || []).map((f) => {
            ...
            return (
              <div key={f.key}>
                ...
                {f.type === "arrangement" && (
                  <ArrangementField value={Array.isArray(val) ? val : []} onChange={commit} terms={t} />
                )}
                {f.type === "symptoms" && (
